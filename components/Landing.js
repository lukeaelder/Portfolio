...
const Landing = ({scrollTo}) => {
    ...
    useLayoutEffect(() => {
        ...
        tl.current.fromTo(name.words, {translateY: 120, autoAlpha: 0}, {autoAlpha: 1, translateY: 0, duration: 1.3, stagger: 0.04, ease: "power3.out"}, 2.2);
        tl.current.fromTo(sub.words, {translateY: 140, autoAlpha: 0}, {autoAlpha: 1, translateY: 0, duration: 1.3, stagger: 0.02, ease: "power3.out"}, 2.2);
    ...
    return (
        <div id="home" ref={el} className="w-full h-[115vh] relative">
            <div className="landing-title flex flex-col items-center absolute top-[25%] w-full font-silka text-neutral-100" data-speed="0.6">
                ...
                <h2 className="landing-sub text-[clamp(20px,2vw,32px)]">Full Stack Software Developer</h2>
            </div>
